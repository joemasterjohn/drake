...
class SpeculativeContactSurface {
 public:
  ...
  const std::vector<Vector3<T>>& p_AoAp_W() const { return p_AoAp_W_; }
  const std::vector<Vector3<T>>& p_BoBq_W() const { return p_BoBq_W_; }
 ...
 private:
  ...
  std::vector<Vector3<T>> p_AoAp_W_;
  std::vector<Vector3<T>> p_BoBq_W_;
