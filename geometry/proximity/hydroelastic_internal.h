...
class SoftMesh {
 public:
  ...
  SoftMesh(std::unique_ptr<VolumeMesh<double>> mesh,
    ...
    mesh_topology_ = std::make_unique<VolumeMeshTopology<double>>(mesh_.get());
  }
  ...
  const std::vector<int>& element_index_mapping() const {
    return element_index_mapping_;
  }
  const VolumeMeshTopology<double>& mesh_topology() const {
 ...
 private:
  ...
  std::unique_ptr<VolumeMeshTopology<double>> mesh_topology_;
  std::vector<int> element_index_mapping_;
};
