...
void VolumeIntersector<MeshBuilder, BvType>::IntersectFields(
    ...
    const std::vector<int>& element_mapping_M,
    const VolumeMeshTopology<double>& mesh_topology_M,
    const VolumeMeshFieldLinear<double, double>& field1_N,
    const Bvh<Obb, TriangleSurfaceMesh<double>>& bvh1_N,
    const std::vector<int>& element_mapping_N,
    const VolumeMeshTopology<double>& mesh_topology_N,
    const math::RigidTransform<T>& X_MN,
    std::unique_ptr<MeshType>* surface_01_M,
    std::unique_ptr<FieldType>* e_01_M) {
  DRAKE_DEMAND(surface_01_M != nullptr);
  ...
  auto callback = [&candidate_tetrahedra, &element_mapping_M,
    ...
    candidate_tetrahedra.emplace(element_mapping_M[tri0],
                                 element_mapping_N[tri1]);
    return BvttCallbackResult::Continue;
  };
...
void HydroelasticVolumeIntersector<MeshBuilder>::IntersectCompliantVolumes(
  ...
  volume_intersector.IntersectFields(
      field0_M, bvh0_M, element_mapping_M, mesh_topology_M, field1_N, bvh1_N,
      element_mapping_N, mesh_topology_N, X_MN, &surface01_M, &field01_M);
...
ComputeContactSurfaceFromCompliantVolumesWithTopology(
  ...
  if (representation == HydroelasticContactRepresentation::kTriangle) {
    HydroelasticVolumeIntersector<TriMeshBuilder<T>>()
        .IntersectCompliantVolumes(id0, field0_M, bvh0_M, element_mapping_M,
                                   mesh_topology_M, X_WM, id1, field1_N, bvh1_N,
                                   element_mapping_N, mesh_topology_N, X_WN,
                                   &contact_surface_W);
  } else {
    HydroelasticVolumeIntersector<PolyMeshBuilder<T>>()
        .IntersectCompliantVolumes(id0, field0_M, bvh0_M, element_mapping_M,
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(
    (&CalcEquilibriumPlane<T>, &IntersectTetrahedra<T>,
     ...
     &ComputeContactSurfaceFromCompliantVolumesWithTopology<T>));
