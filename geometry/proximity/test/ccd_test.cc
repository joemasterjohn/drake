...
CreateVertexFaceTetPair(const RigidTransform<T> X_WG, const T desired_distance,
                        int closest_vertex, std::array<int, 3> closest_face) {
  const Vector3<T> p_GAp(0, 0, 0);
  const Vector3<T> p_GAq(-1, 1, 0);
  const Vector3<T> p_GAr(-1, -0.5, -0.5);
  const Vector3<T> p_GAs(-1, -0.5, 0.5);
  ...
  std::array<Vector3<T>, 4> tet_A = {X_WG * p_GAp, X_WG * p_GAq, X_WG * p_GAr,
                                     X_WG * p_GAs};
  std::swap(
      tet_A[0],
...
template <typename T>
std::pair<std::array<Vector3<T>, 4>, std::array<Vector3<T>, 4>>
CreateEdgeEdgeTetPair(const RigidTransform<T> X_WG, const T desired_distance,
                      int edge_A, int edge_B) {
  const Vector3<T> p_GAp(0, 0, -1);
  const Vector3<T> p_GAq(0, 0, 1);
  const Vector3<T> p_GAr(-1, -1, 0);
  const Vector3<T> p_GAs(-1, 1, 0);
  ...
  tet_A[e_A[0]] = X_WG * p_GAp;
  tet_A[e_A[1]] = X_WG * p_GAq;
  tet_A[opposite_e_A[0]] = X_WG * p_GAr;
  tet_A[opposite_e_A[1]] = X_WG * p_GAs;
...
TYPED_TEST(TetTetDistanceTests, Basic) {
  using T = TypeParam;
  const T kEps{1e1 * std::numeric_limits<double>::epsilon()};

  for (int iterations = 0; iterations < 20; ++iterations) {
    // Generate random transforms and distances
    const T desired_distance = 0.5;
    ...
    for (int i = 0; i < 4; ++i) {
      for (const std::array<int, 3> t : triangles) {
        const auto [tet_A, tet_B] =
            CreateVertexFaceTetPair(X_WA, desired_distance, i, t);
    ...
    for (int i = 0; i < 6; ++i) {
      for (int j = 0; j < 6; ++j) {
        ...
        const auto [tet_A, tet_B] =
            CreateEdgeEdgeTetPair(X_WA, desired_distance, i, j);
        ClosestPointResult<T> result =
            ClosestPointTetrahedronToTetrahedron(tet_A, tet_B);
        EXPECT_LE((result.squared_dist - desired_distance * desired_distance),
                  kEps);
