...
----------------------------------------------------------------------------------------------------
// NOLINT(*) Benchmark Time             CPU   Iterations   Line Number  //
NOLINT(*)
----------------------------------------------------------------------------------------------------
// NOLINT(*) MeshIntersectionBenchmark/RigidSoftMesh/0/4/0/min_time:2.000 0.629
ms        0.629 ms         4408   [14]         // NOLINT(*)
MeshIntersectionBenchmark/RigidSoftMesh/1/4/0/min_time:2.000       2.15 ms 2.15
ms         1328   [15]         // NOLINT(*)
MeshIntersectionBenchmark/RigidSoftMesh/2/4/0/min_time:2.000       3.14 ms 3.14
ms          893   [16]         // NOLINT(*)
MeshIntersectionBenchmark/RigidSoftMesh/3/4/0/min_time:2.000       14.3 ms 14.3
ms          192   [17]         // NOLINT(*)
MeshIntersectionBenchmark/RigidSoftMesh/2/0/0/min_time:2.000      0.000 ms 0.000
ms     54932081   [18]         // NOLINT(*)
MeshIntersectionBenchmark/RigidSoftMesh/2/1/0/min_time:2.000      0.023 ms 0.023
ms       119450   [19]         // NOLINT(*)
MeshIntersectionBenchmark/RigidSoftMesh/2/2/0/min_time:2.000      0.119 ms 0.119
ms        23142   [20]         // NOLINT(*)
MeshIntersectionBenchmark/RigidSoftMesh/2/3/0/min_time:2.000       1.87 ms 1.87
ms         1486   [21]         // NOLINT(*)
MeshIntersectionBenchmark/RigidSoftMesh/2/4/1/min_time:2.000       2.95 ms 2.95
ms          944   [22]         // NOLINT(*)
MeshIntersectionBenchmark/RigidSoftMesh/2/4/2/min_time:2.000       3.15 ms 3.15
ms          894   [23]         // NOLINT(*)
MeshIntersectionBenchmark/RigidSoftMesh/2/4/3/min_time:2.000       3.21 ms 3.21
ms          871   [24]         // NOLINT(*)
MeshIntersectionBenchmark/RigidSoftMesh/2/3/1/min_time:2.000       1.94 ms 1.94
ms         1448   [25]         // NOLINT(*)
MeshIntersectionBenchmark/RigidSoftMesh/2/2/2/min_time:2.000      0.127 ms 0.127
ms        22007   [26]         // NOLINT(*) Resulting contact surface sizes:
...
const double kSphereDimension = 1.;
const double kMaxRotationFactor = 3.;
const double kResolutionHint[4] = {3., 1., 0.5, 0.1};
const Vector3d kContactOverlapTranslation[5] = {
    Vector3d{5, 0, 0},      // 0: No overlap at all.
    Vector3d{1.9, 1.9, 0},  // 1: Overlapping bounding volumes.
    Vector3d{1.99, 0, 0},   // 2: Minimal contact surface.
    Vector3d{1.8, 0, 0},    // 3: Intermediate sized contact surface.
    Vector3d{0, 0, 0}};     // 4: Maximal contact surface.
const TessellationStrategy kTesselationStrategy[2] = {
...
class MeshIntersectionBenchmark : public benchmark::Fixture {
 public:
  MeshIntersectionBenchmark()
      : sphereS_{kSphereDimension},
        sphereR_{kSphereDimension},
        mesh_S_(MakeSphereVolumeMesh<double>(
            sphereS_, 1, TessellationStrategy::kDenseInteriorVertices)),
        field_S_(MakeSpherePressureField<double>(sphereS_, &mesh_S_,
                                                 kElasticModulus)),
        mesh_R_(MakeSphereVolumeMesh<double>(
            sphereR_, 1, TessellationStrategy::kDenseInteriorVertices)),
        field_R_(MakeSpherePressureField<double>(sphereR_, &mesh_R_,
                                                 kElasticModulus)) {}

  /* Parse arguments from the benchmark state.
  @return A tuple representing the resolution, the contact overlap, the
          rotation factor, and the tesselation strategy.  */
  ...
  void SetupMeshes(const benchmark::State& state) {
    const auto [resolution, contact_overlap, rotation_factor, tesselation] =
        ReadState(state);
    const double resolution_hint = kResolutionHint[resolution];
    const TessellationStrategy tesselation_strategy =
        kTesselationStrategy[tesselation];
    mesh_S_ = MakeSphereVolumeMesh<double>(sphereS_, resolution_hint,
                                           tesselation_strategy);
    field_S_ =
        MakeSpherePressureField<double>(sphereS_, &mesh_S_, kElasticModulus);
    mesh_R_ = MakeSphereVolumeMesh<double>(sphereR_, resolution_hint,
                                           tesselation_strategy);
    field_R_ =
        MakeSpherePressureField<double>(sphereR_, &mesh_R_, kElasticModulus);
  ...
  void RecordContactSurfaceResult(const PolygonSurfaceMesh<double>* surface_SR,
    ...
    const std::string result_key = fmt::format(
        "{}/{}/{}/{}/{}", test_name, resolution, contact_overlap, rotation_factor, tesselation);
  ...
  VolumeMesh<double> mesh_R_;
  VolumeMeshFieldLinear<double, double> field_R_;
...
BENCHMARK_DEFINE_F(MeshIntersectionBenchmark, SoftSoftMesh)
// NOLINTNEXTLINE(runtime/references)
(benchmark::State& state) {
  SetupMeshes(state);
  const auto bvh_S = Bvh<Aabb, VolumeMesh<double>>(mesh_S_);
  const auto bvh_R = Bvh<Aabb, VolumeMesh<double>>(mesh_R_);
  std::vector<Vector3d> grad_eS;
  std::vector<Vector3d> grad_eR;
  ...
  for (auto _ : state) {
    IntersectFields<PolygonSurfaceMesh<double>, PolyMeshBuilder<double>>(
        field_S_, bvh_S, field_R_, bvh_R, X_SR_, &surface_SR, &e_SR, &grad_eS,
        &grad_eR);
  }
  RecordContactSurfaceResult(surface_SR.get(), "SoftSoftMesh", state);
}
BENCHMARK_REGISTER_F(MeshIntersectionBenchmark, SoftSoftMesh)
    ->Unit(benchmark::kMillisecond)
    ->MinTime(2)
    ->Args({3, 3, 0, 0})   // 3 resolution, 3 contact overlap, 0 rotation factor, dense interior tesselation.
    ->Args({3, 3, 0, 1})   // 3 resolution, 3 contact overlap, 0 rotation factor, single interior tesselation.
    ->Args({3, 3, 1, 0})   // 3 resolution, 3 contact overlap, 1 rotation factor, dense interior tesselation.
    ->Args({3, 3, 1, 1})   // 3 resolution, 3 contact overlap, 1 rotation factor, single interior tesselation.
    ->Args({3, 3, 2, 0})   // 3 resolution, 3 contact overlap, 2 rotation factor, dense interior tesselation.
    ->Args({3, 3, 2, 1})   // 3 resolution, 3 contact overlap, 2 rotation factor, single interior tesselation.
    ->Args({3, 3, 3, 0})   // 3 resolution, 3 contact overlap, 3 rotation factor, dense interior tesselation.
    ->Args({3, 3, 3, 1})   // 3 resolution, 3 contact overlap, 3 rotation factor, single interior tesselation.
    ->Args({2, 3, 0, 0})   // 2 resolution, 3 contact overlap, 0 rotation factor, dense interior tesselation.
    ->Args({2, 3, 0, 1})   // 2 resolution, 3 contact overlap, 0 rotation factor, single interior tesselation.
    ->Args({2, 3, 1, 0})   // 2 resolution, 3 contact overlap, 1 rotation factor, dense interior tesselation.
    ->Args({2, 3, 1, 1})   // 2 resolution, 3 contact overlap, 1 rotation factor, single interior tesselation.
    ->Args({2, 3, 2, 0})   // 2 resolution, 3 contact overlap, 2 rotation factor, dense interior tesselation.
    ->Args({2, 3, 2, 1})   // 2 resolution, 3 contact overlap, 2 rotation factor, single interior tesselation.
    ->Args({2, 3, 3, 0})   // 2 resolution, 3 contact overlap, 3 rotation factor, dense interior tesselation.
    ->Args({2, 3, 3, 1});  // 2 resolution, 3 contact overlap, 3 rotation factor, single interior tesselation.

    // ->Args({0, 4, 0, 0})   // 0 resolution, 4 contact overlap, 0 rotation factor, dense interior tesselation.
    // ->Args({0, 4, 0, 1})   // 0 resolution, 4 contact overlap, 0 rotation factor, single interior tesselation.
    // ->Args({1, 4, 0, 0})   // 1 resolution, 4 contact overlap, 0 rotation factor, dense interior tesselation.
    // ->Args({1, 4, 0, 1})   // 1 resolution, 4 contact overlap, 0 rotation factor, single interior tesselation.
    // ->Args({2, 4, 0, 0})   // 2 resolution, 4 contact overlap, 0 rotation factor, dense interior tesselation.
    // ->Args({2, 4, 0, 1})   // 2 resolution, 4 contact overlap, 0 rotation factor, single interior tesselation.
    // ->Args({3, 4, 0, 0})   // 3 resolution, 4 contact overlap, 0 rotation factor, dense interior tesselation.
    // ->Args({3, 4, 0, 1})   // 3 resolution, 4 contact overlap, 0 rotation factor, single interior tesselation.
    // ->Args({2, 0, 0, 0})   // 2 resolution, 0 contact overlap, 0 rotation factor, dense interior tesselation.
    // ->Args({2, 0, 0, 1})   // 2 resolution, 0 contact overlap, 0 rotation factor, single interior tesselation.
    // ->Args({2, 1, 0, 0})   // 2 resolution, 1 contact overlap, 0 rotation factor, dense interior tesselation.
    // ->Args({2, 1, 0, 1})   // 2 resolution, 1 contact overlap, 0 rotation factor, single interior tesselation.
    // ->Args({2, 2, 0, 0})   // 2 resolution, 2 contact overlap, 0 rotation factor, dense interior tesselation.
    // ->Args({2, 2, 0, 1})   // 2 resolution, 2 contact overlap, 0 rotation factor, single interior tesselation.
    // ->Args({2, 3, 0, 0})   // 2 resolution, 3 contact overlap, 0 rotation factor, dense interior tesselation.
    // ->Args({2, 3, 0, 1})   // 2 resolution, 3 contact overlap, 0 rotation factor, single interior tesselation.
    // ->Args({2, 4, 1, 0})   // 2 resolution, 4 contact overlap, 1 rotation factor, dense interior tesselation.
    // ->Args({2, 4, 1, 1})   // 2 resolution, 4 contact overlap, 1 rotation factor, single interior tesselation.
    // ->Args({2, 4, 2, 0})   // 2 resolution, 4 contact overlap, 2 rotation factor, dense interior tesselation.
    // ->Args({2, 4, 2, 1})   // 2 resolution, 4 contact overlap, 2 rotation factor, single interior tesselation.
    // ->Args({2, 4, 3, 0})   // 2 resolution, 4 contact overlap, 3 rotation factor, dense interior tesselation.
    // ->Args({2, 4, 3, 1})   // 2 resolution, 4 contact overlap, 3 rotation factor, single interior tesselation.
    // ->Args({2, 3, 1, 0})   // 2 resolution, 3 contact overlap, 1 rotation factor, dense interior tesselation.
    // ->Args({2, 3, 1, 1})   // 2 resolution, 3 contact overlap, 1 rotation factor, single interior tesselation.
    // ->Args({2, 2, 2, 0})   // 2 resolution, 2 contact overlap, 2 rotation factor, dense interior tesselation.
    // ->Args({2, 2, 2, 1});  // 2 resolution, 2 contact overlap, 2 rotation factor, single interior tesselation.
