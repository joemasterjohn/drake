function [error_q, error_v] = calc_error(data)
  % calculate the error based on a reference trajectory located at data(end)
  s = size(data,2);
  error_q = [];
  error_v = [];
  for i=1:s-1
    ref_q = interp1(data(end).t, data(end).q, data(i).t, "linear", "extrap");
    ref_v = interp1(data(end).t, data(end).v, data(i).t, "linear", "extrap");
    error_q(end+1) = trajectory_error(ref_q, data(i).q);
    error_v(end+1) = trajectory_error(ref_v, data(i).v);
  end
end
