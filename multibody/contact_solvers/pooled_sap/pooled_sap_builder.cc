...
void PooledSapBuilder<T>::UpdateModel(const systems::Context<T>& context,
  ...
  // For now just hard code to Log Barrier patches. Later add a parameter to the
  // model to switch between hydro and log-barrier.
  // AddPatchConstraintsForHydroelasticContact(context, model);
  AddPatchConstraintsForLogBarrierContact(context, model);
...
template <typename T>
void PooledSapBuilder<T>::AddPatchConstraintsForLogBarrierContact(
    const systems::Context<T>& context, PooledSapModel<T>* model) const {
  using std::log;
  // Add contact constraints for log-barrier.
  const std::vector<geometry::ContactSurface<T>>& surfaces = scratch_.surfaces;
