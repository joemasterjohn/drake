...
class SapFixedTendonConstraint final : public SapConstraint<T> {
  ...
  class Parameters {
   ...
   private:
    ...
    /* Rigid approximation constant: Rₙ = β²/(4π²)⋅w when the constraint
     ...
     for each constraint. See [Castro et al., 2021] for details. */
    double beta_{0.1};
  };
  ...
  class Kinematics {
   public:
    ...
    /* Constructs a valid set of kinematics representing the tendon length
     ...
     @pre q1.size() == clique1_nv
     @pre a0.size() == clique0_nv
     @pre a0.size() == clique1_nv */
    ...
    /* Single clique constructor.
     ...
     @pre q0.size() == clique0_nv
     @pre a0.size() == clique0_nv */
    Kinematics(int clique0, int clique0_nv, Eigen::SparseVector<T> q0,
    ...
    const Eigen::SparseVector<T>& q0() const { return q0_; }
    const Eigen::SparseVector<T>& q1() const { return q1_; }
    const Eigen::SparseVector<T>& a0() const { return a0_; }
    const Eigen::SparseVector<T>& a1() const { return a1_; }
   ...
   private:
    ...
    int clique1_nv_{-1};
    /* q(0) of clique 0. */
    Eigen::SparseVector<T> q0_;
    /* q(0) of clique 1. */
    Eigen::SparseVector<T> q1_;
    /* Vector of coefficients for clique 0. */
    Eigen::SparseVector<T> a0_;
    /* Vector of coefficients for clique 1. */
    Eigen::SparseVector<T> a1_;
 ...
 private:
  /* Private constructor used by DoToDouble().
   ...
   @pre 0 < g.size() <= 2.
   @pre g.size() == J.rows(). */
  SapFixedTendonConstraint(VectorX<T> g, SapConstraintJacobian<T> J,
