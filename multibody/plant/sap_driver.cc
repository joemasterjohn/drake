...
void SapDriver<T>::AddWeldConstraints(
  ...
  for (const auto& [id, spec] : manager().weld_constraints_specs()) {
    ...
    const math::RigidTransform<T>& X_WFp = X_WA * spec.X_AFp.template cast<T>();
    const math::RigidTransform<T>& X_WFq = X_WB * spec.X_BFq.template cast<T>();
    ...
    manager().internal_tree().CalcJacobianSpatialVelocity(
        ...
        spec.X_AFp.translation().template cast<T>(), frame_W, frame_W,
        &Jv_WFp_W);
    manager().internal_tree().CalcJacobianSpatialVelocity(
        context, JacobianWrtVariable::kV, body_B.body_frame(),
        spec.X_BFq.translation().template cast<T>(), frame_W, frame_W,
        &Jv_WFq_W);
